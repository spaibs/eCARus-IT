...
void NavigationJNIAccess::onCityIndex(int32_t cityIndex)
{
#ifdef ANDROID
    JNIEnv *env = getEnv();
    if(env)
    {
        jmethodID mid = env->GetStaticMethodID(gClsAP, "onCityIndex", "(I)V");
        if (mid != NULL) 
        {
            // Just call the void method without parameters
            env->CallStaticVoidMethod(gClsAP, mid, cityIndex);
        }
        else
        {
            LOGCAT_ERROR("onCityIndex() - GetMethodID failed!");
        }
    }
#else
    // If not Android -> Nothing to do here
