                           ...
                           const GtfFunctor0<void>& c_rCallback)
{
    ...
    if (m_fRunning == false)
    {
        ...
        else
        {
			...
			//
            m_socketListener.init(m_pDataPool, m_pEvSystem);
            m_socketTalker.init(m_pDataPool);
