...
###################################################################
# CREATE_VS_FILTER( ProjectDir FILE1 [FILE2 ...] )
# This macro takes a list of files as input and generates source
# groups that follow the relative path of the files from the
# ProjectDir folder. This is strictly a cosmetic change, but it's
# nice if you're using the CMake generated projects also for
# development
#
# Usage Example:
#  CREATE_VS_FILTER( MyLib/src/ ${MyLibSource} )
#  ADD_LIBRARY( MyLib SHARED ${MyLibSource} )
#
MACRO ( CREATE_VS_FILTER ProjectDir )
    FOREACH(Source ${ARGN})
        set(RelativePath ${Source})
        if(IS_ABSOLUTE ${RelativePath})
           file(RELATIVE_PATH RelativePath ${ProjectDir} ${Source})
        endif(IS_ABSOLUTE ${RelativePath})
        STRING ( REGEX REPLACE "[\\\\/][^\\\\/]*$" "" RelativePath "${RelativePath}" )
        STRING ( REGEX REPLACE "^[\\\\/]" "" RelativePath "${RelativePath}" )
        STRING ( REGEX REPLACE "/" "\\\\\\\\" RelativePath "${RelativePath}" )
        SOURCE_GROUP ( "${RelativePath}" FILES ${Source} )
   ENDFOREACH(Source ${ARGN})
ENDMACRO ( CREATE_VS_FILTER )
...
macro(eb_add_library name)
    PARSE_ARGUMENTS(EB_ADD_LIB
         "INCLUDE_PATH_VARS;INCLUDE_PATHS;SOURCES;NO_FAST_BUILD_SOURCES;LIB_VARS;LIB_PATH_VARS;LIBS;CONVENIENCE_TARGETS"
         ""
         ${ARGN}
         )
   ...
   # message(STATUS "Toolchain uses gnu archiver instead of linker for for creating ${name}")
    ...
    IF(EBFASTBUILD_${name} OR EBFASTBUILD_GLOBAL_ON)
        ...
        create_vs_filter(${PROJECT_SOURCE_DIR}
                         ${EB_ADD_LIB_SOURCES}
...
macro(eb_add_convenience_library name)
    ...
    PARSE_ARGUMENTS(EB_ADD_LIB
         "INCLUDE_PATH_VARS;INCLUDE_PATHS;SOURCES;NO_FAST_BUILD_SOURCES;LIB_VARS;LIB_PATH_VARS;LIBS;SHARED_DEFINITIONS;STATIC_DEFINITIONS"
         ""
         ${ARGN}
         )
...
macro(eb_add_executable name)
    PARSE_ARGUMENTS(EB_ADD_PRG
         "INCLUDE_PATH_VARS;INCLUDE_PATHS;LIB_PATH_VARS;LIB_VARS;SOURCES;NO_FAST_BUILD_SOURCES;LIBS"
         ""
         ${ARGN}
         )
...
macro(eb_add_component name atf_init_src)
    parse_arguments(eb_atf_LIB
         "INCLUDE_PATH_VARS;INCLUDE_PATHS;SOURCES;NO_FAST_BUILD_SOURCES;LIB_VARS;LIB_PATH_VARS;LIBS"
         ""
         ${ARGN}
         )
...
macro(eb_replace_cmake_flags fromvar tovar)
    eb_replace_flags(CMAKE_CXX_FLAGS "${fromvar}" "${tovar}")
    eb_replace_flags(CMAKE_CXX_FLAGS_DEBUG "${fromvar}" "${tovar}")
    eb_replace_flags(CMAKE_CXX_FLAGS_RELEASE "${fromvar}" "${tovar}")
    eb_replace_flags(CMAKE_C_FLAGS "${fromvar}" "${tovar}")
    eb_replace_flags(CMAKE_C_FLAGS_DEBUG "${fromvar}" "${tovar}")
    eb_replace_flags(CMAKE_C_FLAGS_RELEASE "${fromvar}" "${tovar}")
endmacro(eb_replace_cmake_flags)

macro(eb_add_compile_flags_for_file thefile tovar)
    set_property(SOURCE "${thefile}" PROPERTY COMPILE_FLAGS "${tovar}")
endmacro(eb_add_compile_flags_for_file thefile)
