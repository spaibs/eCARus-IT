...
elseif(IS_DIRECTORY "$ENV{HOME}/android/android-ndk")
    set(NDK "$ENV{HOME}/android/android-ndk")
elseif(IS_DIRECTORY "/opt/android/android-ndk")
    set(NDK "/opt/android/android-ndk")
elseif(IS_DIRECTORY "/usr/local/android/android-ndk")
    set(NDK "/usr/local/android/android-ndk")
else()
...
SET(ANDROID_NDK_PREBUILT "windows-x86_64" CACHE STRING "Android NDK prebuilt platform")
SET(ANDROID_NDK_ANDROID_API "android-14" CACHE STRING "Android API level")
SET(ANDROID_NDK_TOOLCHAIN_EABI "arm-linux-androideabi-4.8" CACHE STRING "Android NDK toolchain")

SET(CMAKE_NDK_SOURCES "${ANDROID_NDK_ROOT}/sources")
SET(CMAKE_NDK_NATIVE_APP_GLUE "${CMAKE_NDK_SOURCES}/android/native_app_glue")
SET(CMAKE_NDK_GNU_LIBSTDC "${CMAKE_NDK_SOURCES}/cxx-stl/gnu-libstdc++/4.8")
...
SET(CMAKE_NDK_OGLES_LIB "${CMAKE_NDK_SYSROOT}/usr/lib/libGLESv2.so")
SET(GTF3DRENDERER_OPENGLES20_EGL_LIBRARY "${CMAKE_NDK_OGLES_EGL_LIB}")
SET(GTF3DRENDERER_OPENGLES20_OGLES2_INCLUDE_DIR "${CMAKE_NDK_OGLES_INCLUDE_DIR}")
SET(GTF3DRENDERER_OPENGLES20_OGLES2_LIBRARY "${CMAKE_NDK_OGLES_LIB}")

#
# The android NDK compiler
#
SET(CMAKE_CXX_COMPILER "${CMAKE_NDK_TOOLCHAIN_HOST}/bin/arm-linux-androideabi-g++.exe")
SET(CMAKE_C_COMPILER "${CMAKE_NDK_TOOLCHAIN_HOST}/bin/arm-linux-androideabi-gcc.exe")

#
# Override the default linker
#
#SET(CMAKE_CXX_LINK_EXECUTABLE "${CMAKE_NDK_TOOLCHAIN_HOST}/bin/arm-linux-androideabi-ld.exe")
#SET(CMAKE_C_LINK_EXECUTABLE "${CMAKE_NDK_TOOLCHAIN_HOST}/bin/arm-linux-androideabi-ld.exe")
...
# Compiler flags
#
SET(CMAKE_CXX_FLAGS "${CMAKE_NDK_CC_FLAGS} -fno-exceptions -fno-rtti" CACHE STRING "C++ Compiler flags")
SET(CMAKE_C_FLAGS "${CMAKE_NDK_CC_FLAGS}" CACHE STRING "C Compiler flags")

#
# Linker flags
#
set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_NDK_EXE_FLAGS} ${CMAKE_NDK_ALLOW_SHLIB_UNDEF} -L${CMAKE_NDK_GNU_LIBSTDC}/libs/armeabi-v7a -lgnustl_shared" CACHE STRING "Executable linker flags")
