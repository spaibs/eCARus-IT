...
void TowerAccessJNIManager::playTitle(char *title)
{
#ifdef ANDROID
	JNIEnv *env = getEnv();
	if (env)
	{
		...
		if (mid != NULL)
		{
			//jstring titleParameter = (jstring) "test";
			jstring titleParameter = env->NewStringUTF(title);
			//jstring titleParameter = env->NewStringUTF(env,title);
...
void TowerAccessJNIManager::pauseTitle()
{
#ifdef ANDROID
	JNIEnv *env = getEnv();
	if (env)
