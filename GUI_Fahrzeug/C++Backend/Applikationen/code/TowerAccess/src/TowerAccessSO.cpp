...
void TowerAccessMsgReceiver::startup(GtfPluginLoader& rLoader)
{
...
void TowerAccessMsgReceiver::shutdown()
{
...
void TowerAccessMsgReceiver::HandleInterface(const uint32_t c_interfaceId,
...
uint16_t TowerAccessMsgReceiver::HandleRunlevel(const uint16_t c_runlevel,
                                               const int8_t c_direction)
{
        ...
        if (c_runlevel == EMPTY_APP_SERVICE_STARTUP_RUNLEVEL)
        {
                ...
                && (m_validRunlevel <= c_runlevel))
            {
                ...
                fSuccess = m_service.Startup(*m_pCoreModel,
                                         *m_pWorkLoop,
                                         gtf_bind(&TowerAccessMsgReceiver::UnlockRunlevel, this));
            }
            if (fSuccess == false)
            {
    ...
    if (c_direction < 0)
    {
        ...
        {
            LockRunlevel();
            m_service.Shutdown(gtf_bind(&TowerAccessMsgReceiver::UnlockRunlevel, this));
            m_pCoreModel = NULL;
        }
    }
...
void TowerAccessMsgReceiver::HandleMessage(const GtfMsgHandle c_msg)
{
    GTF_UNUSED_PARAM(c_msg);
}

void TowerAccessMsgReceiver::LockRunlevel()
{
...
void TowerAccessMsgReceiver::UnlockRunlevel()
{
...
void TowerAccessMsgReceiver::RequestShutdownOnFailure()
{
...
GTF_PLUGIN_SO_SYMBOL GtfPlugin * GTF_CREATE_PLUGIN_SYMBOL(uint32_t argc, char** argv)
{
    ...
    return GtfPluginFactory<TowerAccessMsgReceiver>::create();
}

GTF_PLUGIN_SO_SYMBOL void GTF_DESTROY_PLUGIN_SYMBOL(GtfPlugin * const c_pPlugin)
{
    GtfPluginFactory<TowerAccessMsgReceiver>::destroy(c_pPlugin);
}
