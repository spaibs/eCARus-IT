                           ...
                           const GtfFunctor0<void>& c_rCallback)
{
    ...
    if (m_fRunning == false)
    {
        ...
        else
        {
			...
			//
            MusicServiceAccessJNIManager* manager = MusicServiceAccessJNIManager::getInstance();
            manager->registerObserver(this);
            manager->getTrackList();
...
void MusicServiceAccess::ProcessEvent(GtfEventHandle handle)
{
    ...
    if((true == ev.GetGroupId(grpId)) && (true == ev.GetEventId(evId)))
    {
        ...
        LOGCAT_DEBUG("grpId : %i evId %i", grpId, evId);
		
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		if ((grpId == EVENT_GRP_ID_ExtOut_Media_PlayTitle) && (evId == EVENT_MSG_ID_ExtOut_Media_PlayTitle))
        {
            int32_t trackIndex = 0;
            bool success = ev.ReadValue(0, GtfTypeId::eTypeId_int32, &trackIndex, sizeof(int32_t));

			LOGCAT_DEBUG("play title");
			
            if(success)
            {
				MusicServiceAccessJNIManager::getInstance()->playTitle(trackIndex);
            }
        }
                                 ...
                                 const uint32_t c_itemCount)
{
    ...
    for (uint32_t index = 0; index < c_itemCount; index++)
    {
        ...
        {
        case DP_ID_ExtOut_MediaPlayer_Volume:

            float volumeValue;
            m_pDataPool->Scalar_ReadValue(DP_CONTEXT_ID,
                DP_ID_ExtOut_MediaPlayer_Volume,
                GtfTypeId::eTypeId_float,
                &volumeValue,
                sizeof(float));

            MusicServiceAccessJNIManager::getInstance()->updateVolume(volumeValue);
...
uint32_t MusicServiceAccess::getTrackPropertyId(GtfDataPool** pDataPool, GtfTypesDP::contextId_t* ctxId)
{
    *pDataPool = m_pDataPool;
    *ctxId = DP_CONTEXT_ID;

    return DP_ID_ExtIn_MediaPlayer_TitlelList;
}
