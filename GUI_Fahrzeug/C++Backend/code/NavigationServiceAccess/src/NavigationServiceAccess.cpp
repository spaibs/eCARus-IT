                           ...
                           const GtfFunctor0<void>& c_rCallback)
{
    ...
    if (m_fRunning == false)
    {
        ...
        fSuccess = fSuccess && m_pEvSystem->Subscribe(DP_CONTEXT_ID,
            EVENT_GRP_ID_ExtOut_Navigation_Select_City,
            EVENT_MSG_ID_ExtOut_Navigation_Select_City,
            SubscribeToEvent);
...
void NavigationServiceAccess::ProcessEvent(GtfEventHandle handle)
{
    ...
    { 
		...
		else if ((grpId == EVENT_GRP_ID_ExtOut_Navigation_Select_City) && (evId == EVENT_MSG_ID_ExtOut_Navigation_Select_City))
        {
            int32_t cityIndex = 0;
      

            // Read event parameters and store to local variables
            bool success = ev.ReadValue(0, GtfTypeId::eTypeId_int32, &cityIndex, sizeof(int32_t));

            if(success) // If reading was successful...
            {
                // Forward touch parameters to JNI access class
                NavigationJNIAccess::getInstance().onCityIndex(cityIndex);
            }
        }
    }
}
